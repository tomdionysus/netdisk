...
static int netdisk_open(struct block_device *disk, fmode_t mode) {
  if (!blk_get_queue(disk->bd_queue)) {
...
static void netdisk_release(struct gendisk *disk, fmode_t mode) { blk_put_queue(disk->part0->bd_queue); }
