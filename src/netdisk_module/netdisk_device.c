...
static int netdisk_process_request(struct request *rq) {
    ...
    rq_for_each_segment(bvec, rq, iter) {
      ...
      while (b_len) {
        ...
        if (create_chunk(trans, block_id, b_buf, chunk_size) != 0) {
        ...
        packet->fields.block_offset = block_id;
        packet->fields.user_data = transaction_id;

        if(rq_data_dir(rq) == WRITE) {
            packet->fields.command = NETDISK_COMMAND_WRITE;
            memcpy(packet->fields.payload, b_buf, chunk_size);
        } else {
            packet->fields.command = NETDISK_COMMAND_READ;
        }
        if(packet_send(packet, config.key, &config.address) != NETDISK_PACKET_STATUS_OK) {
             printk(KERN_ALERT "netdisk: packet send NETDISK_COMMAND_START failed\n");
        }
        kfree(packet);
...
int create_netdisk_device(char *devicename) {
    ...
    if (dev->data == NULL) {
        printk(KERN_ERR "netdisk: Failed to allocate device IO buffer\n");
        release_netdisk_device();
