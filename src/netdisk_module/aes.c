...
void AES_init_ctx(struct AES_ctx* ctx, const u8* key) { KeyExpansion(ctx->RoundKey, key); }
void AES_init_ctx_iv(struct AES_ctx* ctx, const u8* key, const u8* iv) {
  KeyExpansion(ctx->RoundKey, key);
  memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}
void AES_ctx_set_iv(struct AES_ctx* ctx, const u8* iv) { memcpy(ctx->Iv, iv, AES_BLOCKLEN); }
